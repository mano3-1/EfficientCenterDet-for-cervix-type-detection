...
def loss_fn(gamma1,gamma2):
    ...
    def final_loss(y_true,y_pred):
        ...
        loss = (8*cls_loss+3*offloss+5*size_loss)
        return loss
    return final_loss
...
def lrs(epoch):
    ...
    return lr
reduce_lr_loss = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=6, verbose=1, epsilon=1e-3, mode='min')  
...
#checkpoints
checkpoint = ModelCheckpoint('/home/b170007ec/Programs/Manoj/DETECTOR/model2-{epoch:03d}-{val_loss:03f}.h5',monitor = 'val_loss', save_best_only = True, verbose = 1, period = 1)
#training
history = model.fit_generator(train_gen,epochs = n_epochs ,verbose = DISPLAY,steps_per_epoch = STEPS_PER_EPOCH,validation_data = val_gen,shuffle = True,validation_steps = len(curV)//(BATCH_SIZE*4),callbacks = [reduce_lr_loss,checkpoint,early_stopping])